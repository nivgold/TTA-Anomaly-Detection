...
class AnnthyroidDataset:
    def __init__(self, data_dir_path, from_disk=False, disk_path='/home/nivgold/pkls/oversampling_pkls/annthyroid_pkls'):
        ...
        if from_disk:
            ...
            # loading from files
            self.features_full = pd.read_pickle(disk_path + "/features_full_annthyroid.pkl")
            self.labels_full = pd.read_pickle(disk_path + "/labels_full_annthyroid.pkl")

            self.X_pairs = np.load(self.disk_path + "/X_pairs_annthyroid.npy")
            self.y_pairs = np.load(self.disk_path + "/y_pairs_annthyroid.npy")

        else:
            file_name = 'annthyroid.csv'
    ...
    def save_attributes_to_disk(self):
        ...
        # save train full
        pd.to_pickle(self.features_full, self.disk_path + "/features_full_annthyroid.pkl")
        pd.to_pickle(self.labels_full, self.disk_path + "/labels_full_annthyroid.pkl")
        # save siamese pairs
        np.save(self.disk_path + "/X_pairs_annthyroid.npy", self.X_pairs)
        np.save(self.disk_path + "/y_pairs_annthyroid.npy", self.y_pairs)


def get_dataset(data_path, batch_size, from_disk=True):
    annthyroid = AnnthyroidDataset(data_path, from_disk=from_disk)

    return annthyroid.features_full.values, annthyroid.labels_full.values, (annthyroid.X_pairs, annthyroid.y_pairs)
