...
class MammoDataset:
    def __init__(self, data_dir_path, from_disk=False, disk_path='/home/nivgold/pkls/oversampling_pkls/mammo_pkls'):
        ...
        if from_disk:
            ...
            self.X_pairs = np.load(self.disk_path + "/X_pairs_mammo.npy")
            self.y_pairs = np.load(self.disk_path + "/y_pairs_mammo.npy")
    ...
    def save_attributes_to_disk(self):
        ...
        # save siamese pairs
        np.save(self.disk_path + "/X_pairs_mammo.npy", self.X_pairs)
        np.save(self.disk_path + "/y_pairs_mammo.npy", self.y_pairs)


def get_dataset(data_path, batch_size, from_disk=True):
    mammo = MammoDataset(data_path, from_disk=from_disk)

    return mammo.features_full.values, mammo.labels_full.values, (mammo.X_pairs, mammo.y_pairs)
