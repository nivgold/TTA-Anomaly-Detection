...
class MuskDataset:
    def __init__(self, data_dir_path, from_disk=False, disk_path='/home/nivgold/pkls/oversampling_pkls/musk_pkls'):
        ...
        if from_disk:
            ...
            # loading from files
            self.features_full = pd.read_pickle(disk_path + "/features_full_musk.pkl")
            self.labels_full = pd.read_pickle(disk_path + "/labels_full_musk.pkl")

            self.X_pairs = np.load(self.disk_path + "/X_pairs_musk.npy")
            self.y_pairs = np.load(self.disk_path + "/y_pairs_musk.npy")
    ...
    def preprocessing(self, data):
        ...
        def _agg_df(df):
            ...
            desired_cols = [str(i) for i in np.arange(166)]
    ...
    def save_attributes_to_disk(self):
        ...
        # save train full
        pd.to_pickle(self.features_full, self.disk_path + "/features_full_musk.pkl")
        pd.to_pickle(self.labels_full, self.disk_path + "/labels_full_musk.pkl")
        # save siamese pairs
        np.save(self.disk_path + "/X_pairs_musk.npy", self.X_pairs)
        np.save(self.disk_path + "/y_pairs_musk.npy", self.y_pairs)


def get_dataset(data_path, batch_size, from_disk=True):
    musk = MuskDataset(data_path, from_disk=from_disk)

    return musk.features_full.values, musk.labels_full.values, (musk.X_pairs, musk.y_pairs)
