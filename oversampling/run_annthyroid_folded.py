...
import annthyroid_dataset_folded as satmd
...
annthyroid_X, annthyroid_y, annthyroid_pairs = satmd.get_dataset(HOME_PATH, 32, from_disk=True)
end_train_test = time.time()
print("--- Annthyroid dataset ready after: ", end='')
get_execute_time(start_time, end_train_test)
dataset_name = 'annthyroid'
weights_path = '/home/nivgold/models/oversampling_models'
solver_obj = Solver(X=annthyroid_X, y=annthyroid_y, n_folds=args.n_folds, dataset_name=dataset_name, epochs=EPOCHS, features_dim=ANNTHYROID_DIM, siamese_data=annthyroid_pairs)
