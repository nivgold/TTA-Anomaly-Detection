...
cardio_train_ds, cardio_test_ds, cardio_features_full = carmd.get_dataset(HOME_PATH, 32, from_disk=True)
end_train_test = time.time()
print("---Cardio train_ds, test_ds ready after: ", end='')
get_execute_time(start_time, end_train_test)
solver_obj = Solver(cardio_train_ds, cardio_test_ds, epochs=EPOCHS, features_dim=CARDIO_DIM)
dataset_name = 'cardio'
...
accuracy, precision, recall, f_score, auc = solver_obj.test_tta(oversampling_method, num_neighbors=num_neighbors, num_augmentations=num_augmentations, knn_data=cardio_features_full)
