...
vowels_train_ds, vowels_test_ds, vowels_features_full, vowels_pairs = vomd.get_dataset(HOME_PATH, 32, from_disk=True)
end_train_test = time.time()
print("--- Vowels dataset ready after: ", end='')
get_execute_time(start_time, end_train_test)
dataset_name = 'vowels'
solver_obj = Solver(vowels_train_ds, vowels_test_ds, dataset_name=dataset_name, epochs=EPOCHS, features_dim=VOWELS_DIM, knn_data=vowels_features_full, siamese_data=vowels_pairs)
