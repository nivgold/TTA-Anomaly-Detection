...
yeast_train_ds, yeast_test_ds, yeast_features_full, yeast_pairs = yemd.get_dataset(HOME_PATH, 32, from_disk=True)
end_train_test = time.time()
print("--- Yeast dataset ready after: ", end='')
get_execute_time(start_time, end_train_test)
dataset_name = 'yeast'
solver_obj = Solver(yeast_train_ds, yeast_test_ds, dataset_name=dataset_name, epochs=EPOCHS, features_dim=YEAST_DIM, knn_data=yeast_features_full, siamese_data=yeast_pairs)
