...
class SatimageDataset:
    def __init__(self, data_dir_path, from_disk=False, disk_path='/home/nivgold/pkls/oversampling_pkls/satimage_pkls'):
        ...
        if from_disk:
            ...
            # loading from files
            self.train_features = pd.read_pickle(disk_path+"/train_features_satimage.pkl")
            self.train_labels = pd.read_pickle(disk_path+"/train_labels_satimage.pkl")

            self.test_features = pd.read_pickle(disk_path+"/test_features_satimage.pkl")
            self.test_labels = pd.read_pickle(disk_path+"/test_labels_satimage.pkl")

            self.features_full = pd.read_pickle(disk_path + "/features_full_satimage.pkl")
            self.labels_full = pd.read_pickle(disk_path + "/labels_full_satimage.pkl")

            self.X_pairs = np.load(self.disk_path + "/X_pairs_satimage.npy")
            self.y_pairs = np.load(self.disk_path + "/y_pairs_satimage.npy")

        else:
            file_name = 'satimage.csv'
    ...
    def preprocessing(self, data):
        ...
        def _agg_df(df):
            ...
            desired_cols = [str(i) for i in range(36)]
    ...
    def save_attributes_to_disk(self):
        ...
        # save train
        pd.to_pickle(self.train_features, self.disk_path+"/train_features_satimage.pkl")
        pd.to_pickle(self.train_labels, self.disk_path+"/train_labels_satimage.pkl")
        # save test
        pd.to_pickle(self.test_features, self.disk_path+"/test_features_satimage.pkl")
        pd.to_pickle(self.test_labels, self.disk_path+"/test_labels_satimage.pkl")
        # save train full
        pd.to_pickle(self.features_full, self.disk_path + "/features_full_satimage.pkl")
        pd.to_pickle(self.labels_full, self.disk_path + "/labels_full_satimage.pkl")
        # save siamese pairs
        np.save(self.disk_path + "/X_pairs_satimage.npy", self.X_pairs)
        np.save(self.disk_path + "/y_pairs_satimage.npy", self.y_pairs)
...
def get_dataset(data_path, batch_size, from_disk=True):
    ...
    satimage = SatimageDataset(data_path, from_disk=from_disk)
    ...
    train_ds = (tf.data.Dataset.from_tensor_slices(
        (dict(satimage.train_features), satimage.train_labels))
                .cache()
                .batch(batch_size)
    ...
    test_ds = (tf.data.Dataset.from_tensor_slices(
        (dict(satimage.test_features), satimage.test_labels))
               .cache()
               .batch(batch_size)
               .map(test_pack_features_vector))

    return train_ds, test_ds, satimage.features_full.values, (satimage.X_pairs, satimage.y_pairs)
