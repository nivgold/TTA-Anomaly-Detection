...
class ShuttleDataset:
    def __init__(self, data_dir_path, from_disk=False, disk_path='/home/nivgold/pkls/oversampling_pkls/shuttle_pkls'):
        ...
        if from_disk:
            ...
            # loading from files
            self.train_features = pd.read_pickle(disk_path+"/train_features_shuttle.pkl")
            self.train_labels = pd.read_pickle(disk_path+"/train_labels_shuttle.pkl")

            self.test_features = pd.read_pickle(disk_path+"/test_features_shuttle.pkl")
            self.test_labels = pd.read_pickle(disk_path+"/test_labels_shuttle.pkl")

            self.features_full = pd.read_pickle(disk_path + "/features_full_shuttle.pkl")
            self.labels_full = pd.read_pickle(disk_path + "/labels_full_shuttle.pkl")

            self.X_pairs = np.load(self.disk_path + "/X_pairs_shuttle.npy")
            self.y_pairs = np.load(self.disk_path + "/y_pairs_shuttle.npy")

        else:
            file_name = 'shuttle.csv'
    ...
    def preprocessing(self, data):
        ...
        def _agg_df(df):
            ...
            desired_cols = ['0', '1', '2', '3', '4', '5', '6', '7', '8']
    ...
    def save_attributes_to_disk(self):
        ...
        # save train
        pd.to_pickle(self.train_features, self.disk_path+"/train_features_shuttle.pkl")
        pd.to_pickle(self.train_labels, self.disk_path+"/train_labels_shuttle.pkl")
        # save test
        pd.to_pickle(self.test_features, self.disk_path+"/test_features_shuttle.pkl")
        pd.to_pickle(self.test_labels, self.disk_path+"/test_labels_shuttle.pkl")
        # save train full
        pd.to_pickle(self.features_full, self.disk_path + "/features_full_shuttle.pkl")
        pd.to_pickle(self.labels_full, self.disk_path + "/labels_full_shuttle.pkl")
        # save siamese pairs
        np.save(self.disk_path + "/X_pairs_shuttle.npy", self.X_pairs)
        np.save(self.disk_path + "/y_pairs_shuttle.npy", self.y_pairs)
...
def get_dataset(data_path, batch_size, from_disk=True):
    ...
    shuttle = ShuttleDataset(data_path, from_disk=from_disk)
    ...
    train_ds = (tf.data.Dataset.from_tensor_slices(
        (dict(shuttle.train_features), shuttle.train_labels))
                .cache()
                .batch(batch_size)
    ...
    test_ds = (tf.data.Dataset.from_tensor_slices(
        (dict(shuttle.test_features), shuttle.test_labels))
               .cache()
               .batch(batch_size)
               .map(test_pack_features_vector))

    return train_ds, test_ds, shuttle.features_full.values, (shuttle.X_pairs, shuttle.y_pairs)
