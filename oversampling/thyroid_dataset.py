...
class ThyroidDataset:
    def __init__(self, data_dir_path, from_disk=False, disk_path='/home/nivgold/pkls/oversampling_pkls/thyroid_pkls'):
        ...
        if from_disk:
            ...
            # loading from files
            self.train_features = pd.read_pickle(disk_path+"/train_features_thyroid.pkl")
            self.train_labels = pd.read_pickle(disk_path+"/train_labels_thyroid.pkl")

            self.test_features = pd.read_pickle(disk_path+"/test_features_thyroid.pkl")
            self.test_labels = pd.read_pickle(disk_path+"/test_labels_thyroid.pkl")

            self.features_full = pd.read_pickle(disk_path + "/features_full_thyroid.pkl")
            self.labels_full = pd.read_pickle(disk_path + "/labels_full_thyroid.pkl")

            self.X_pairs = np.load(self.disk_path + "/X_pairs_thyroid.npy")
            self.y_pairs = np.load(self.disk_path + "/y_pairs_thyroid.npy")
    ...
    def save_attributes_to_disk(self):
        ...
        # save train
        pd.to_pickle(self.train_features, self.disk_path+"/train_features_thyroid.pkl")
        pd.to_pickle(self.train_labels, self.disk_path+"/train_labels_thyroid.pkl")
        # save test
        pd.to_pickle(self.test_features, self.disk_path+"/test_features_thyroid.pkl")
        pd.to_pickle(self.test_labels, self.disk_path+"/test_labels_thyroid.pkl")
        # save train full
        pd.to_pickle(self.features_full, self.disk_path + "/features_full_thyroid.pkl")
        pd.to_pickle(self.labels_full, self.disk_path + "/labels_full_thyroid.pkl")
        # save siamese pairs
        np.save(self.disk_path + "/X_pairs_thyroid.npy", self.X_pairs)
        np.save(self.disk_path + "/y_pairs_thyroid.npy", self.y_pairs)
...
def get_dataset(data_path, batch_size, from_disk=True):
    ...
    thyroid = ThyroidDataset(data_path, from_disk=from_disk)
    ...
    train_ds = (tf.data.Dataset.from_tensor_slices(
        (dict(thyroid.train_features), thyroid.train_labels))
                .cache()
                .batch(batch_size)
    ...
    test_ds = (tf.data.Dataset.from_tensor_slices(
        (dict(thyroid.test_features), thyroid.test_labels))
               .cache()
               .batch(batch_size)
               .map(test_pack_features_vector))

    return train_ds, test_ds, thyroid.features_full.values, (thyroid.X_pairs, thyroid.y_pairs)
