...
class YeastDataset:
    def __init__(self, data_dir_path, from_disk=False, disk_path='/home/nivgold/pkls/oversampling_pkls/yeast_pkls'):
        ...
        if from_disk:
            ...
            # loading from files
            self.train_features = pd.read_pickle(disk_path+"/train_features_yeast.pkl")
            self.train_labels = pd.read_pickle(disk_path+"/train_labels_yeast.pkl")

            self.test_features = pd.read_pickle(disk_path+"/test_features_yeast.pkl")
            self.test_labels = pd.read_pickle(disk_path+"/test_labels_yeast.pkl")
    ...
    def save_attributes_to_disk(self):
        ...
        # save train
        pd.to_pickle(self.train_features, self.disk_path+"/train_features_yeast.pkl")
        pd.to_pickle(self.train_labels, self.disk_path+"/train_labels_yeast.pkl")
        # save test
        pd.to_pickle(self.test_features, self.disk_path+"/test_features_yeast.pkl")
        pd.to_pickle(self.test_labels, self.disk_path+"/test_labels_yeast.pkl")
        # save train full
...
def get_dataset(data_path, batch_size, from_disk=True):
    ...
    train_ds = (tf.data.Dataset.from_tensor_slices(
        (dict(yeast.train_features), yeast.train_labels))
                .cache()
                .batch(batch_size)
    ...
    test_ds = (tf.data.Dataset.from_tensor_slices(
        (dict(yeast.test_features), yeast.test_labels))
               .cache()
               .batch(batch_size)
               .map(test_pack_features_vector))

    return train_ds, test_ds, yeast.features_full.values, (yeast.X_pairs, yeast.y_pairs)
