...
class KaggleDataset:
    def __init__(self, data_dir_path, from_disk=False, disk_path='/home/nivgold/pkls/sliding_window_pkls/kaggle_pkls'):
        self.disk_path = disk_path

        if from_disk:

            # loading from files
            self.train_features = pd.read_pickle(self.disk_path+"/train_features_kaggle.pkl")
            self.train_labels = pd.read_pickle(self.disk_path+"/train_labels_kaggle.pkl")

            self.test_features = pd.read_pickle(self.disk_path+"/test_features_kaggle.pkl")
            self.test_labels = pd.read_pickle(self.disk_path+"/test_labels_kaggle.pkl")

            self.tta_features = list(np.load(self.disk_path+"/tta_features_kaggle.npy"))
            self.tta_labels = list(np.load(self.disk_path+"/tta_labels_kaggle.npy"))

        else:
            data_file_name = "TimeSeries.csv"
            labels_file_name = "labelsTimeSeries.csv"
            print(f"Openning {data_file_name}")
            data_path = os.path.join(data_dir_path, data_file_name)
            labels_path = os.path.join(data_dir_path, labels_file_name)
            data_df = pd.read_csv(data_path, encoding='cp1252')
            labels_df = pd.read_csv(labels_path, encoding='cp1252')
    ...
    def preprocessing(self, data):
        ...
        def _normalize_columns(df):
            df_labels = df["label"]
            df_features = df.drop(columns=["label"], axis=1)
        ...
        def _agg_df(df):
            ...
            # taking all columns but the last one - the label column
            desired_cols = df.columns[:-1]
    ...
    def save_attributes_to_disk(self):
        ...
        # save train
        pd.to_pickle(self.train_features, self.disk_path+"/train_features_kaggle.pkl")
        pd.to_pickle(self.train_labels, self.disk_path+"/train_labels_kaggle.pkl")
        # save test
        pd.to_pickle(self.test_features, self.disk_path+"/test_features_kaggle.pkl")
        pd.to_pickle(self.test_labels, self.disk_path+"/test_labels_kaggle.pkl")
        # save tta
        np.save(self.disk_path+"/tta_features_kaggle.npy", self.tta_features)
        np.save(self.disk_path+"/tta_labels_kaggle.npy", self.tta_labels)
...
def get_dataset(data_path, batch_size, from_disk=True):
    ...
    kaggle = KaggleDataset(data_path, from_disk=from_disk)
    ...
    train_ds = (tf.data.Dataset.from_tensor_slices((dict(kaggle.train_features), kaggle.train_labels))
                .cache()
                .batch(batch_size)
    ...
    test_ds = (tf.data.Dataset.from_tensor_slices(
        (dict(kaggle.test_features), kaggle.test_labels, kaggle.tta_features, kaggle.tta_labels))
               .cache()
               .batch(batch_size)
